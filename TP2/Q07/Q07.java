...
class Game{
    ...
    public String toString(){
        String str = this.app_id + " " + 
                     ...
                     this.owners + " " + 
                     this.age + " " + 
                     String.format("%.2f", this.price).replace(",", ".") + " " +
                     this.dlcs + " [" + 
...
class Queue{
    ...
    public void print(){
        for(int i = this.first, j = 0; i != this.last; i = (i + 1) % this.games.length, j++){
            MyIO.println("[" + j + "] " + this.games[i].toString());
        }
    }

    public Game get(int index){
        if(index < this.length && index >= 0){
            return this.games[(index + this.first) % this.games.length];
        }
    ...
    public void push(Game game){
        ...
        this.games[this.last] = game;
        
        this.last = (this.last + 1) % this.games.length;
        this.length++;
    }

    public Game pop(){
        if(this.length > 0){
            Game game = this.games[this.first];
            this.first = (this.first + 1) % this.games.length;
            this.length--;
...
public class Q07{
    ...
    public static void main(String[] args){
                ...
                for(int i = 0; i < gamesArray.length; i++){
                    if(gamesArray[i].getApp_id() == app_id){
                        ...
                        for(int j = 0; j < games.length(); j++){
                            avg_avg_pt += games.get(j).getAvg_pt();
                        }

                        MyIO.println(Math.round(avg_avg_pt / games.length()));
                    }
                }
            }
        }while(!stop);
        ...
        for(int i = 0; i < n; i++){
            ...
            if(str.charAt(0) == 'I'){
                int app_id = Integer.parseInt(str.substring(2, str.length()));
                MyIO.println("I " + app_id);
