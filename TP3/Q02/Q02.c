...
void matriculaSelecaoRecursiva(int comp, int mov, int time){
    FILE* f = fopen("matricula_selecaoRecursiva.txt", "w");
...
int main(){
    ...
    clock_t start = clock();
    rec_selection_sort(&list, 0, 1, 0);
    clock_t end = clock();

    print_list(&list);

    matriculaSelecaoRecursiva(list.comp, list.mov, end - start);
